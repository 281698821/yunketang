...
// layui初始化模块使用
layui.use(['table', 'hpWindow','utils','hpFormAll'], function () {
    ...
    var cmd = {
        ...
        initEvent: function () {
            ...
            dataTable=table.render({
                elem: '#dataTable'
                ,url:'/positionController/ajaxList'
                ,title: '用户数据表'
                ,page: true
                ,cols: [[
                    {type: 'checkbox', fixed: 'left'},
                    {field: 'name', align: 'center', title: '职位'},
                    {field: 'mark', align: 'center', title: '备注'},
                    {field: 'dept', templet: cmd.fmt.deptFmt, align: 'center', title: '所属部门'},
            ...
            table.on('tool(tableTool)', function (obj) {
                ...
                if (obj.event === 'del') {
                    ...
                    hpFormAll.open(".updateBtn","/positionController/saveUpdatePage?id=" + data.id,'修改职位信息');
                    /*hpWindow.openOne(".updateBtn", "/positionController/saveUpdatePage?id=" + data.id, {
                        title: '修改',
                        area: ['550px', '400px'],
                        btn: ['确认', '取消'],
                        anim: 1,
        ...
        bindEvent: function () {
            ...
            $("#export").click(function () {
                location.href = '/positionController/export';
            });
            // 批量删除
            $('#deletesBtn').click(function () {
                layer.confirm('批量删除吗', function (index) {
                    ...
                    if (checkStatus) {
                        ...
                        ids = arr.join(',');
                        $.ajax({
                            url: '/positionController/deletes',
                            data: {"ids": ids},
                            type: 'post',
                            dataType: 'json',
                            success: function (json) {
        ...
        initOpenWindow: function () {
            hpFormAll.open("#addBtn","/positionController/saveUpdatePage",'添加职位信息');
            // 添加弹窗
           /* hpWindow.open("#addBtn", "/positionController/saveUpdatePage", {
                title: '添加',
                area: ['550px', '400px'],
                btn: ['确认', '取消'],
                anim: 1,
